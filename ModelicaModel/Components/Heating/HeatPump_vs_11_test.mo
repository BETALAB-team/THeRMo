...
model HeatPump_vs_11_test "Deleted control on frrquency of compressors because the esperimental one is used for the model validation"
  Heating.Ref_cycle_vs_9 ref_cycle_vs_9(
    fileName=fileName,
    SH=SH,
    SBC=SBC,
  ...
  Heating.CMP_poly_vs_8 CMP_8(fileName=fileName, CMP_type=CMP_type) annotation (Placement(transformation(extent={{-40,-18},{0,12}})));
  Heating.Condenser_vs_3 Condenser(
    ...
    V=V_cond) annotation (Placement(transformation(extent={{32,54},{62,84}})));
  ...
  Heating.Dew_T_calculation DewT(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{2,-64},{-14,-48}})));
  ...
  parameter Modelica.Units.SI.Volume V_cond
                                           "Volume"
    annotation (Dialog(group="Condenser"));
...
equation
  ...
  connect(ref_cycle_vs_9.Wel_ref, Wel_ref) annotation (Line(points={{63.91,8.73},
          {100,8.73},{100,24},{110,24}}, color={0,0,127}));

  connect(ref_cycle_vs_9.COP, COP) annotation (Line(points={{63.91,-14.05},{88,
          -14.05},{88,-14},{110,-14}},                      color={0,0,127}));
  connect(ref_cycle_vs_9.HC_ref, Condenser.HeatFlow) annotation (Line(points={{43,
          16.38},{43.4,16.38},{43.4,52.2}},
                                        color={0,0,127}));
  connect(Load_in, Condenser.port_a)
    annotation (Line(points={{-70,110},{-70,69},{32,69}}, color={0,127,255}));
  connect(Condenser.port_b, Load_out)
    annotation (Line(points={{62,69},{72,69},{72,110}}, color={0,127,255}));
  connect(Evaporator.port_a, Source_in) annotation (Line(points={{28,-71},{28,
          -72},{-70,-72},{-70,-110}},
                                 color={0,127,255}));
  connect(Evaporator.port_b, Source_out) annotation (Line(points={{58,-71},{58,
          -70},{70,-70},{70,-110}},
                               color={0,127,255}));
  connect(Condenser.RefT,CMP_8. Tcond) annotation (Line(points={{30.5,63},{-20,
  ...
  connect(ref_cycle_vs_9.CC_ref, gain.u) annotation (Line(points={{43,-22.38},{
          43,-31.4}},              color={0,0,127}));
  connect(Evaporator.RefT, DewT.T_eva_in) annotation (Line(points={{26.5,-63.5},
          {26.5,-56},{3.6,-56}},  color={0,0,127}));
  connect(gain.y, Evaporator.HeatFlow)
    annotation (Line(points={{43,-38.3},{37.45,-38.3},{37.45,-54.95}},
                                                     color={0,0,127}));
  connect(DewT.T_dew, ref_cycle_vs_9.Teva) annotation (Line(points={{-15.44,-56},
          {-20,-56},{-20,-30},{0,-30},{0,-13.2},{23.96,-13.2}}, color={0,0,127}));
  connect(CMP_8.Mode_Output, ref_cycle_vs_9.OnOff) annotation (Line(points={{-5,
          -3.15},{-2,-3},{23.96,-3}}, color={255,127,0}));
  connect(CMP_8.Wel, ref_cycle_vs_9.Wel_map) annotation (Line(points={{-4.8,1.2},
          {-2,1.76},{23.96,1.76}}, color={0,0,127}));
  connect(CMP_8.CC, ref_cycle_vs_9.CC_map) annotation (Line(points={{-4.8,-7.5},
          {-2,-8.1},{23.96,-8.1}}, color={0,0,127}));
  connect(ref_cycle_vs_9.M_ref, Condenser.m_ref) annotation (Line(points={{56.26,
          16.38},{52.85,16.38},{52.85,52.35}},
                                            color={0,0,127}));
  connect(Condenser.HC_secondary, division.u1) annotation (Line(points={{63.5,
          62.7},{72,62.7},{72,45.2},{78.6,45.2}}, color={0,0,127}));
  connect(ref_cycle_vs_9.Wel_ref, division.u2) annotation (Line(points={{63.91,
          8.73},{72,8.73},{72,36.8},{78.6,36.8}},
                                            color={0,0,127}));
  ...
  connect(Evaporator.port_a, SET.port) annotation (Line(points={{28,-71},{28,
          -72},{-52,-72},{-52,-54}},
                                color={0,127,255}));
  connect(SET.T, ref_cycle_vs_9.SET) annotation (Line(points={{-45,-44},{14,-44},
          {14,-18.3},{23.96,-18.3}}, color={0,0,127}));
  connect(ref_cycle_vs_9.Tcond, CMP_8.Tcond) annotation (Line(points={{23.96,
          7.2},{0,7.2},{0,26},{-20,26},{-20,13.8}},
                                               color={0,0,127}));
  connect(LET.T, ref_cycle_vs_9.LET) annotation (Line(points={{-47,50},{14,50},
          {14,12.3},{23.96,12.3}},color={0,0,127}));
  connect(Load_in, LET.port)
    annotation (Line(points={{-70,110},{-70,40},{-54,40}}, color={0,127,255}));
