...
model Condenser_vs_3_UAeff
  ...
  Modelica.Blocks.Interfaces.RealOutput RefT(unit="K") "Medium temperature"
    annotation (Placement(transformation(extent={{-10,-10},{10,10}},
        rotation=180,
        origin={-114,-72}), iconTransformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={-114,-72})));
  ...
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor Tcond
...
equation
  ...
  connect(heaFlo.port_b, vol.heatPort) annotation (Line(points={{-24,-12},{-24,-10},
          {-9,-10}},                color={191,0,0}));
  connect(preHeaFlo.Q_flow, HeatFlow) annotation (Line(points={{-22,-94},{-22,-104},
          {-24,-104},{-24,-112}},color={0,0,127}));
  connect(port_a,SensEF. port)
    annotation (Line(points={{-100,0},{-62,0},{-62,32}}, color={0,127,255}));
  connect(SensExF.port, vol.ports[3])
    annotation (Line(points={{40,24},{40,0},{1,0}}, color={0,127,255}));
  connect(preHeaFlo.port, con.solid)
    annotation (Line(points={{-22,-74},{-22,-58}}, color={191,0,0}));
  connect(heaFlo.port_a, con.fluid)
    annotation (Line(points={{-24,-28},{-24,-38},{-22,-38}},
                                                   color={191,0,0}));
  connect(UAeff.y, con.Gc)
    annotation (Line(points={{-51,-48},{-32,-48}}, color={0,0,127}));
  connect(Tcond.port, con.solid) annotation (Line(points={{-50,-72},{-36,-72},{-36,
          -62},{-22,-62},{-22,-58}}, color={191,0,0}));
  connect(Tcond.T, RefT)
    annotation (Line(points={{-71,-72},{-114,-72}}, color={0,0,127}));
  annotation (
