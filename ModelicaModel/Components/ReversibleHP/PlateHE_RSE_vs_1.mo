...
model PlateHE_RSE_vs_1 "Exponential UA formulation as function of mass flow rates"
  ...
  parameter Real m_ref_nom_heat(unit="kg/s") "Nominal refrigerant mass flow rate [kg/s]";
  parameter Real m_ref_nom_cool(unit="kg/s") "Nominal refrigerant mass flow rate [kg/s]";
  parameter Real m_f_nom_heat(unit="kg/s") "Nominal flow mass flow rate heating [kg/s]";
  parameter Real m_f_nom_cool(unit="kg/s") "Nominal flow mass flow rate cooling [kg/s]";
...
equation
  ...
  if HP_operative_status ==1 then
    m_flow_rel = m_flow_safe/m_f_nom_heat;
    m_ref_rel = m_ref/m_ref_nom_heat;
  else
    m_flow_rel = m_flow_safe/m_f_nom_cool;
    m_ref_rel = m_ref/m_ref_nom_cool;
  end if;
