...
model Ref_cycle_reversible_vs_1 "Reversible refrigerant cycle model"
  ...
  Modelica.Blocks.Interfaces.RealInput T_ref_1(unit="K", displayUnit="degC") "condensing temperature - saturated vapor condenser temperature for zeotropic mixtures"
    annotation (Placement(transformation(extent={{-124,48},{-100,72}}), iconTransformation(extent={{-124,48},{-100,72}})));
  Modelica.Blocks.Interfaces.RealInput T_ref_2(unit="K", displayUnit="degC") "evaporating temperature (dew point)"
  ...
  Modelica.Blocks.Interfaces.RealInput  CC_Wel[2]( unit="W") "Cooling capacity and Wel in standardised conditions"
    annotation (Placement(transformation(extent={{-122,-32},{-100,-10}}), iconTransformation(extent={{-122,-32},{-100,-10}})));
  ...
  Real Teva( unit="K") "evaporating temperature ";
  Real Tcond( unit="K") "condensing temperature ";
  ...
  Real HC_ref(unit="W") "Heating capacity";
  Real CC_ref(unit="W") "Cooling capcaity capacity";
  Real CC_map(unit="W") "Cooling capacity in standard conditions";

   //--------------Refrigerant Ouput and blocks-----------------------------------------------------------------------------------------------------------------------------------------------------

  Modelica.Blocks.Interfaces.RealOutput HE1_output[2] "HC refrigerant and refrigerant mass flow rate in heating, else CC and m flow in cooling" annotation (Placement(transformation(
        extent={{-13,-13},{13,13}},
        rotation=90,
  ...
  Modelica.Blocks.Routing.DeMultiplex2 deMultiplex2 annotation (Placement(transformation(
        ...
        rotation=0,
        origin={-66,0})));
  Modelica.Blocks.Interfaces.RealOutput HE2_output(unit="W") "CC calculated with refrigerant cycle in heating., HC in cooling mde" annotation (Placement(transformation(
        extent={{-21,-21},{21,21}},
        rotation=-90,
        origin={-1,-121}), iconTransformation(
        extent={{-12,-12},{12,12}},
        rotation=270,
        origin={0,-114})));
  Modelica.Blocks.Interfaces.IntegerInput HP_operatve_status annotation (Placement(transformation(extent={{-124,8},{-100,32}}), iconTransformation(extent={{-124,8},{-100,32}})));
  Modelica.Blocks.Interfaces.RealOutput Wel_ref(unit="W") "heating capacity calculated with refrigerant cycle"
    annotation (Placement(transformation(extent={{102,48},{144,90}}), iconTransformation(extent={{102,48},{144,90}})));
  Modelica.Blocks.Interfaces.RealOutput HP_efficency "COP in heating, EER in cooling"
...
equation
  ...
  sat2 = Medium.setSat_T(T2_vs) "saturation conditions at condenser vapour inlet";
 ...
 //--------------Isoenropic Enthalpy point 2 - Daibiri correlations--------------------------------------------------------------------------------------------------------------------------------
  ...
  h4_map = Medium.specificEnthalpy_pT(Pcond, T3_map) "enthalpy at the evaporator ineltt in standardised condtions (h3_map = h4_map)";
 ...
 //-------------- Calculation of max T2 temperature------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if HP_operatve_status == 1 then
      HE1_output = {HC_ref,m_ref};
  else
      HE1_output = {CC_ref,m_ref};
  end if;
  ...
  if HP_operatve_status == 1 then
    HP_efficency = Buildings.Utilities.Math.Functions.smoothMax(
    HC_ref/Wel_ref,
    1e-4,
    1e-5) "calculation of coefficent of performance";
  else
    HP_efficency = Buildings.Utilities.Math.Functions.smoothMax(
      HE2_output/Wel_ref,
      1e-4,
      1e-5)
           "calculation of energy efficency ratio";
  end if;
 ...
 //-------------- Calculation of percentage of heat exchanged in condensation and evaporation------------------------------------------------------------------------------------------------------
  ...
  connect(CC_Wel, deMultiplex2.u) annotation (Line(points={{-111,-21},{-80,-21},{-80,0},{-73.2,0}},
                                                                                  color={0,0,127}));
  annotation (
    Icon(graphics={
        Rectangle(
          ...
          lineThickness=1),
        Bitmap(extent={{-96,-84},{94,126}}, fileName="modelica://HeatPumpModel/../Incons/Immagine 2025-11-14 124316.png"),
        Text(
          extent={{-98,-52},{94,-114}},
          textColor={0,0,255},
          textString="Reversible Ref Cycle
