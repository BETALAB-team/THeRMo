...
model Ref_cycle_vs_6 "Add mass flow rates as output"
...
//Define Parameters
 ...
 Real Wel_ref_unb(  unit = "W") "unbonded refrigerant power";
  ...
  Modelica.Blocks.Interfaces.RealOutput M_ref =  m_ref annotation (Placement(
        transformation(extent={{102,-20},{138,16}}), iconTransformation(extent={
            {102,-20},{138,16}})));
equation
  ...
  //Energy balances
  Wel_ref = smooth( 1, noEvent(if OnOff ==1 then m_ref*(h2-h1) else 0)) "calculation of cmp power using the refrigerant cycle";
