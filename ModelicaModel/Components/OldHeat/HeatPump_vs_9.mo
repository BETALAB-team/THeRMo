...
model HeatPump_vs_9
  "New UA correlation for the condenser, filter of the first order directly on the frequency of the compressor"
  ...
  parameter Real yMin=0 "Lower limit of output"
    annotation (Dialog(group="Controller"));
  OldHeat.CMP_poly_vs_7 CMP_7(fileName=fileName, CMP_type=CMP_type)
    annotation (Placement(transformation(extent={{-40,-18},{0,12}})));
  Condenser_variable_UA_rel    Condenser(
  ...
  Heating.OnOff_vs_4 onOff_vs_4_1(f_nominal=f_nominal, T=T_hp)
  ...
  parameter Real UA_nom "Nominal UA value"
    annotation (Dialog(group="Condenser"));
  parameter Modelica.Units.SI.Time T_hp=60 "Time Constant of the Heat Pump"
    annotation (Dialog(group="Controller"));
equation
  ...
  connect(ref_cycle_vs_8.HC_ref, Condenser.HeatFlow) annotation (Line(points={{
          41.17,17.91},{43,17.91},{43,52.2}}, color={0,0,127}));
  ...
  connect(Evaporator.port_b, Source_out) annotation (Line(points={{54,-71},{54,
          -70},{70,-70},{70,-110}},
                               color={0,127,255}));
  connect(Condenser.RefT,CMP_7. Tcond) annotation (Line(points={{65.5,64.5},{-20,
          64.5},{-20,13.8}}, color={0,0,127}));
  connect(Condenser.RefT, ref_cycle_vs_8.Tcond) annotation (Line(points={{65.5,64.5},
          {-10,64.5},{-10,14},{21.96,14},{21.96,12.3}},       color={0,0,127}));
  connect(COP, COP)
    annotation (Line(points={{118,-22},{118,-22}}, color={0,0,127}));
  connect(DewT.T_dew,CMP_7. Teva) annotation (Line(points={{-15.44,-56},{-20,
          -56},{-20,-18}},      color={0,0,127}));
  connect(onOff_vs_4_1.Output,CMP_7. CMP_f) annotation (Line(points={{-43.4,3.32},
          {-40,3.32},{-40,3.15},{-36.2,3.15}}, color={0,0,127}));
  connect(onOff_vs_4_1.OnOff,CMP_7. Mode) annotation (Line(points={{-43.4,-6.2},
          {-43.4,-6},{-40,-6},{-40,-8.55},{-36.2,-8.55}}, color={255,127,0}));
  ...
  connect(DewT.T_dew, ref_cycle_vs_8.Teva) annotation (Line(points={{-15.44,-56},
          {-20,-56},{-20,-26},{12,-26},{12,-17.96},{21.96,-17.96}}, color={0,0,
          127}));
  connect(CMP_7.Mode_Output, ref_cycle_vs_8.OnOff) annotation (Line(points={{-5,
          -3.15},{-2,-3},{21.96,-3}}, color={255,127,0}));
  connect(CMP_7.Wel, ref_cycle_vs_8.Wel_map) annotation (Line(points={{-4.8,1.2},
          {-2,1.76},{21.96,1.76}}, color={0,0,127}));
  connect(CMP_7.CC, ref_cycle_vs_8.CC_map) annotation (Line(points={{-4.8,-7.5},
