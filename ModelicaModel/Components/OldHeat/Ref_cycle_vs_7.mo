...
model Ref_cycle_vs_7
  "Modified input with CC instead of m_ref to adapt the model to the structure of the 20 polynomials coefficients"
...
//Define Parameters
  ...
  Modelica.Blocks.Interfaces.RealOutput HC_ref( unit = "W")
    ...
    annotation (Placement(
        ...
        rotation=-90,
        origin={1,123})));
  Modelica.Blocks.Interfaces.RealInput CC_map(unit="W")
    "Cooling capacity in standardised conditions" annotation (Placement(
        transformation(extent={{-124,-42},{-100,-18}}), iconTransformation(
          extent={{-124,-42},{-100,-18}})));
  Modelica.Blocks.Interfaces.RealOutput CC_ref(unit="W")
    "heating capacity calculated with refrigerant cycle" annotation (Placement(
        transformation(extent={{-21,-21},{21,21}},
        rotation=-90,
        origin={-1,-121}),                            iconTransformation(extent={{-21,-21},
            {21,21}},
        rotation=270,
        origin={1,-123})));
  ...
  Modelica.Blocks.Interfaces.RealOutput Wel_ref(unit="W")
    "heating capacity calculated with refrigerant cycle" annotation (Placement(
        transformation(extent={{102,48},{144,90}}),  iconTransformation(extent={{102,48},
            {144,90}})));

  Modelica.Blocks.Interfaces.RealOutput COP
    "Coeffiicent of performance of the Heat Pump" annotation (Placement(
        transformation(extent={{102,-110},{144,-68}}),iconTransformation(extent={{102,-86},
            {144,-44}})));
 ...
 Real m_ref_map( unit = "kg/s") "refrigerant mass flow rate in standardised conditions";
  ...
  Modelica.Blocks.Interfaces.RealOutput M_ref =  m_ref annotation (Placement(
        transformation(extent={{-20,-20},{20,20}},
        rotation=90,
        origin={78,122}),                            iconTransformation(extent={{-20,-20},
            {20,20}},
        rotation=90,
        origin={78,122})));
equation
  ...
  h4_map = Medium.specificEnthalpy_pT(Pcond,T3_map) "enthalpy at the evaporator outlet in standardised condtions (h3_map = h4_map)";
  h4 = h3 "isoenthalpic lamination";
  T4 = Medium.temperature_ph(Peva,h4) "enthalpy at the condenser outlet";

  // Calculation of m_ref_map
  m_ref_map = CC_map/(h1_map - h4_map) "refrigerant mass flow rates in standard conditions";

  //Max T2 calculation
  m_ref =smooth(1, noEvent(if OnOff == 1 then m_ref_map*(1 + F*(d1/d1_map - 1)) else
                                                                                    1e-4))
                                                                                          "refrigerant mass flow rate calculated at real operative conditions";
  Wel_ref_unb =smooth(1, noEvent(if OnOff == 1 then Wel_map*m_ref*(h2_iso - h1)/(m_ref_map*(h2_iso_map - h1)) else 1e-4))
                                                                                                                         "unbonded eletric power calculated at real operative conditions";
  h2_unb = smooth(1,noEvent(if OnOff ==1 then Wel_ref_unb/m_ref + h1 else max(h1,hcond_vs))) "enthalpy at compressor outlet without upper limitation";
  h2_max = Medium.specificEnthalpy_pT(Pcond, 100 + 273.15, 1) "maximum enthalpy allowed at the outlet of the compressor";
  h2 = smooth(1, noEvent(if OnOff == 1 then  Buildings.Utilities.Math.Functions.smoothMin(h2_unb, h2_max, 1e-4) else hcond_vs)) "real entalpy at the condenser outlet";
  T2 = Medium.temperature_ph(Pcond, h2);

  //Energy balances
  Wel_ref = smooth( 1, noEvent(if OnOff ==1 then m_ref*(h2-h1) else 1e-4)) "calculation of cmp power using the refrigerant cycle";
  HC_ref = smooth( 1, noEvent(if OnOff ==1 then m_ref*(h2-h3) else 1e-4)) "calculation of heating capacity using the refrigerant cycle";
  CC_ref = smooth( 1, noEvent(if OnOff ==1 then m_ref*(h1-h4) else 1e-4)) "calculation of cooling capacity using the refrigerant cycle";
  COP = smooth( 1, noEvent(if OnOff ==1 then HC_ref / Wel_ref else 1e-4)) "Coefficient of performance";

  //Calculation of percentage of heat exchanged in condensation and evaporation
  hcond_vs = sat2.hv;
  hcond_ls = sat2.hl;
  Deltah_cond = (hcond_vs - hcond_ls)/(h2 - h3)
                                               " quantity of heat transferred due to phase change";
...
")}));
  annotation (Icon(graphics={
        Rectangle(
          ...
          fillPattern=FillPattern.Solid,
          lineThickness=1),
        Bitmap(extent={{-96,-74},{94,98}}, fileName=
              "modelica://HeatPumpModel/../Incons/Immagine 2025-11-14 124316.png"),
        Text(
          extent={{-54,-66},{46,-84}},
          textColor={0,0,0},
