...
model CMP_poly_vs_7 "Added softStart"
  ...
  //Import data
   ...
   //First order filter
   Modelica.Blocks.Continuous.FirstOrder firstOrder(T(displayUnit="min") = 60)
   ...
   Real CC_raw "raw CC calculated without filter";

  //Parameter definition
  Modelica.Blocks.Interfaces.IntegerInput Mode "operating mode" annotation (Placement(transformation(extent={{-92,-48},
            {-70,-26}}),iconTransformation(extent={{-92,-48},{-70,-26}})));
...
equation
  if CMP_type == "Fixed speed" then
    Wel = Buildings.Utilities.Math.Functions.smoothMax(firstOrder.y *el_coef * t_vector_fix,0,1e-5);
    CC = Buildings.Utilities.Math.Functions.smoothMax(firstOrder.y *cc_coef * t_vector_fix,0,1e-5);
    HC = CC + Wel;
    CC_raw = cc_coef * t_vector_fix;
    m_ref = Buildings.Utilities.Math.Functions.smoothMax(firstOrder.y *(m_coef* t_vector_fix)/3600,0,1e-5) "report to kg/s";
  elseif CMP_type == "Variable speed 20 coeff" then
    Wel = Buildings.Utilities.Math.Functions.smoothMax( firstOrder.y *el_coef_20 * t_vector_20,0,1e-5);
    CC =  Buildings.Utilities.Math.Functions.smoothMax(firstOrder.y * cc_coef_20 * t_vector_20,0,1e-5);
    CC_raw = cc_coef_20 * t_vector_20;
    HC = CC + Wel;
    m_ref =  Buildings.Utilities.Math.Functions.smoothMax(firstOrder.y *  (m_coef_20* t_vector_20)/3600,0,1e-5) "report to kg/s";
  elseif CMP_type == "Variable speed 30 coeff" then
    Wel = Buildings.Utilities.Math.Functions.smoothMax( firstOrder.y *el_coef_30 * t_vector_30,0,1e-5);
    CC = Buildings.Utilities.Math.Functions.smoothMax( firstOrder.y *cc_coef_30 * t_vector_30,0,1e-5);
    HC = CC + Wel;
    CC_raw = cc_coef_30 * t_vector_30;
    m_ref = Buildings.Utilities.Math.Functions.smoothMax(firstOrder.y *(m_coef_30* t_vector_30)/3600,0,1e-5) "report to kg/s";
  end if;
  connect(integerToReal.y, firstOrder.u)
    annotation (Line(points={{-23,-36},{-14,-36}}, color={0,0,127}));
  connect(integerToReal.u, Mode) annotation (Line(points={{-46,-36},{-66,-36},{-66,
          -37},{-81,-37}}, color={255,127,0}));
