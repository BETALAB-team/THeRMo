...
model Ref_cycle_vs_8
  "Introduced FirstOrder filter to smooth the change from ON to OFF and viceversa"
...
equation
  ...
  h2_unb = Buildings.Utilities.Math.Functions.smoothMax(Wel_ref_unb/max(m_ref,1e-4) + h1,h1,1e-5) "enthalpy at compressor outlet without upper limitation";
  h2_max = Medium.specificEnthalpy_pT(Pcond, 150 + 273.15, 1) "maximum enthalpy allowed at the outlet of the compressor";
  ...
  //Energy balances
  Wel_ref = Buildings.Utilities.Math.Functions.smoothMax(m_ref*(h2-h1),1e-4,1e-5) "calculation of cmp power using the refrigerant cycle";
  HC_ref = Buildings.Utilities.Math.Functions.smoothMax(m_ref*(h2-h3),1e-4,1e-5) "calculation of heating capacity using the refrigerant cycle";
  CC_ref = Buildings.Utilities.Math.Functions.smoothMax(m_ref*(h1-h4),1e-4,1e-5) "calculation of cooling  capacity using the refrigerant cycle";
  COP = Buildings.Utilities.Math.Functions.smoothMax(HC_ref / Wel_ref,1e-4,1e-5) "calculation of coefficent of performance";
