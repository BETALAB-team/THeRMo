...
model CMP_poly_vs_6 "Reading of 20 coefficients polynomials"
...
equation
  if CMP_type == "Fixed speed" then
    Wel = smooth(1,noEvent(if Mode==1 then el_coef * t_vector_fix else 1e-4));
    CC = smooth(1,noEvent(if Mode==1 then cc_coef*t_vector_fix else 1e-4));
    HC = CC + Wel;
    m_ref = smooth(1,noEvent(if Mode==1 then (m_coef* t_vector_fix)/3600 else 1e-4)) "report to kg/s";
  elseif CMP_type == "Variable speed 20 coeff" then
    Wel = smooth(1,noEvent(if Mode==1 then el_coef_20 * t_vector_20 else 1e-4));
    CC = smooth(1,noEvent(if Mode==1 then cc_coef_20*t_vector_20 else 1e-4));
    HC = CC + Wel;
    m_ref = smooth(1,noEvent(if Mode==1 then (m_coef_20* t_vector_20)/3600 else 1e-4)) "report to kg/s";
  elseif CMP_type == "Variable speed 30 coeff" then
    Wel = smooth(1,noEvent(if Mode==1 then el_coef_30 * t_vector_30 else 1e-4));
    CC = smooth(1,noEvent(if Mode==1 then cc_coef_30*t_vector_30 else 1e-4));
    HC = CC + Wel;
    m_ref = smooth(1,noEvent(if Mode==1 then (m_coef_30* t_vector_30)/3600 else 1e-4)) "report to kg/s";
  end if;
