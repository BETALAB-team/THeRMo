...
model HeatPump_vs_1_UAeff_cooling "Usign UA eff instead of inverse calculation"
  ...
  HeatPumpModel.Components.Cooling.Ref_cycle_vs_9_cooling ref_cycle_vs_9_cooling(
  ...
  Evaporator_vs_3_UAeff_cooling                            EvaCool(
  ...
  Condenser_vs_2_UEeff_cooling                            CondCool(
  ...
  Modelica.Fluid.Interfaces.FluidPort_a Load_in( redeclare package Medium = eva_medium_cool) "Fluid connector a (positive design flow direction is from port_a to port_b)" annotation (Placement(transformation(extent={{-114,50},{-94,70}}), iconTransformation(extent={{-120,60},
            {-100,80}})));
  Modelica.Fluid.Interfaces.FluidPort_b Load_out( redeclare package Medium = eva_medium_cool) "Fluid connector b (positive design flow direction is from port_a to port_b)" annotation (Placement(transformation(extent={{94,50},{114,70}}), iconTransformation(extent={{100,60},
            {120,80}})));
  Modelica.Fluid.Interfaces.FluidPort_a Source_in( redeclare package Medium = cond_medium_cool) "Fluid connector a (positive design flow direction is from port_a to port_b)"  annotation (Placement(transformation(extent={{-116,-70},{-96,-50}}), iconTransformation(extent={{-120,
            -80},{-100,-60}})));
  Modelica.Fluid.Interfaces.FluidPort_b Source_out( redeclare package Medium = cond_medium_cool) "Fluid connector b (positive design flow direction is from port_a to port_b)" annotation (Placement(transformation(extent={{94,-70},{114,-50}}), iconTransformation(extent={{100,-80},
            {120,-60}})));
...
equation
  connect(ref_cycle_vs_9_cooling.CC_tot, EvaCool.CC_tot) annotation (Line(points={{45.15,17.95},{44,17.95},{44,46.44}},
                                                                                                                     color={0,0,127}));
  ...
  connect(division.y, EER_system) annotation (Line(points={{86.7,37},{96,37},{96,21},{107,21}},
                                                                                color={0,0,127}));
  connect(ref_cycle_vs_9_cooling.EER, EER) annotation (Line(points={{63.45,-8.75},{63.45,-23},{107,-23}},
                                                                                                        color={0,0,127}));
  connect(EvaCool.Tref, DewT.T_eva_in) annotation (Line(points={{29.7,53.2},{-31,53.2},{-31,47.8}}, color={0,0,127}));
  connect(EvaCool.CC_secondary, division.u1) annotation (Line(points={{58.3,53.2},{64,53.2},{64,41.2},{70.6,41.2}}, color={0,0,127}));
  connect(ref_cycle_vs_9_cooling.Teva, EvaCool.Tref) annotation (Line(points={{28.2,10.3},{-6,10.3},{-6,53.2},{29.7,53.2}}, color={0,0,127}));
  connect(CondCool.RefT, ref_cycle_vs_9_cooling.Tcond) annotation (Line(points={{29.7,-54.5},{-8,-54.5},{-8,-8},{28.2,-8}}, color={0,0,127}));
...
")}));
end HeatPump_vs_1_UAeff_cooling;
