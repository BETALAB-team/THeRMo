...
model CMP_poly_vs_9_cooling "Remove integer"
  ...
  //------------------Fixed speed poly-------------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   Modelica.Blocks.Interfaces.RealInput CMP_f(unit="Hz")
    ...
    annotation (Placement(transformation(extent={{-126,-14},{-100,12}}),
...
equation
  ...
  if CMP_f < minF then
    ...
    if CMP_type == "Fixed speed" then
      Wel = Buildings.Utilities.Math.Functions.smoothMax(el_coef * t_vector_fix,1e-4,1e-5);
      CC = Buildings.Utilities.Math.Functions.smoothMax(cc_coef * t_vector_fix,1e-4,1e-5);
      HC = CC + Wel;
    elseif CMP_type == "Variable speed 20 coeff" then
      Wel = Buildings.Utilities.Math.Functions.smoothMax(el_coef_20 * t_vector_20,1e-4,1e-5);
      CC =  Buildings.Utilities.Math.Functions.smoothMax(cc_coef_20 * t_vector_20,1e-4,1e-5);
      HC = CC + Wel;
    elseif CMP_type == "Variable speed 30 coeff" then
      Wel = Buildings.Utilities.Math.Functions.smoothMax( el_coef_30 * t_vector_30,1e-4,1e-5);
      CC = Buildings.Utilities.Math.Functions.smoothMax(cc_coef_30 * t_vector_30,1e-4,1e-5);
      HC = CC + Wel;
    end if;
  end if;
