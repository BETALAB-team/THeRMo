...
model HeatPump_vs_2_cooling "Efficency calculated usign refrigerant temperature"
  ...
  OldCool.Ref_cycle_vs_10_cooling ref_cycle_vs_10_cooling(
  ...
  Modelica.Fluid.Interfaces.FluidPort_a Load_in( redeclare package Medium = eva_medium_cool) "Fluid connector a (positive design flow direction is from port_a to port_b)" annotation (Placement(transformation(extent={{-114,50},{-94,70}}), iconTransformation(extent={{-80,100},{-60,120}})));
  Modelica.Fluid.Interfaces.FluidPort_b Load_out( redeclare package Medium = eva_medium_cool) "Fluid connector b (positive design flow direction is from port_a to port_b)" annotation (Placement(transformation(extent={{94,50},{114,70}}), iconTransformation(extent={{62,100},{82,120}})));
  Modelica.Fluid.Interfaces.FluidPort_a Source_in( redeclare package Medium = cond_medium_cool) "Fluid connector a (positive design flow direction is from port_a to port_b)"  annotation (Placement(transformation(extent={{-116,-70},{-96,-50}}), iconTransformation(extent={{-80,-120},{-60,-100}})));
  Modelica.Fluid.Interfaces.FluidPort_b Source_out( redeclare package Medium = cond_medium_cool) "Fluid connector b (positive design flow direction is from port_a to port_b)" annotation (Placement(transformation(extent={{94,-70},{114,-50}}), iconTransformation(extent={{60,-120},{80,-100}})));
  ...
  parameter Modelica.Units.SI.PressureDifference dp__eva_cool_nominal "Pressure difference" annotation (Dialog(group="Evaporator_cooling"));
  ...
  parameter String fileName_eva_cool="C:/Users\benafra10167/Desktop/HE_polynomial/B8T.xlsx" "File path where polynomial coefficients are stored"
                                                                                                                                                annotation (choices(
...
equation
  connect(ref_cycle_vs_10_cooling.CC_tot, EvaCool.CC_tot) annotation (Line(points={{45.15,17.95},{44,17.95},{44,46.44}}, color={0,0,127}));
  connect(CondCool.HeatFlow, ref_cycle_vs_10_cooling.HC_ref) annotation (Line(points={{43.87,-46.57},{45.15,-44},{45.15,-15.95}}, color={0,0,127}));
  connect(cMP_poly_vs_8_cooling.CC_Wel, ref_cycle_vs_10_cooling.CC_Wel) annotation (Line(points={{-11.62,3.2},{-11.62,4},{28.2,4}}, color={0,0,127}));
  connect(cMP_poly_vs_8_cooling.Mode_Output, ref_cycle_vs_10_cooling.OnOff) annotation (Line(points={{-11.79,-3.36},{-11.79,-2},{28.2,-2}}, color={255,127,0}));
  connect(EvaCool.RefT, ref_cycle_vs_10_cooling.Teva) annotation (Line(points={{29.7,55.8},{-12,55.8},{-12,8},{28.2,8},{28.2,10.3}}, color={0,0,127}));
  ...
  connect(CondCool.RefT, ref_cycle_vs_10_cooling.Tcond) annotation (Line(points={{29.7,-54.5},{-12,-54.5},{-12,-8},{28.2,-8}}, color={0,0,127}));
  ...
  connect(SET.T, ref_cycle_vs_10_cooling.SET) annotation (Line(points={{11.6,-32},{20,-32},{20,-12.5},{28.2,-12.5}}, color={0,0,127}));
  connect(LET.T, ref_cycle_vs_10_cooling.LET) annotation (Line(points={{11.6,30},{20,30},{20,14.5},{28.2,14.5}}, color={0,0,127}));
  ...
  connect(DewT.T_dew, cMP_poly_vs_8_cooling.Teva) annotation (Line(points={{-31,26.38},{-31.17,26.38},{-31.17,18.08}},
                                                                                                                   color={0,0,127}));
  connect(ref_cycle_vs_10_cooling.Wel_ref, division.u2) annotation (Line(points={{63.45,11.35},{62,11.35},{62,32.8},{70.6,32.8}}, color={0,0,127}));
  connect(EvaCool.CC_secondary, division.u1) annotation (Line(points={{58.3,55.54},{64,55.54},{64,41.2},{70.6,41.2}}, color={0,0,127}));
  connect(division.y, EER_system) annotation (Line(points={{86.7,37},{96,37},{96,21},{107,21}},
                                                                                color={0,0,127}));
  connect(ref_cycle_vs_10_cooling.EER, EER) annotation (Line(points={{63.45,-8.75},{63.45,-23},{107,-23}}, color={0,0,127}));
  annotation (Icon(graphics={Bitmap(extent={{-80,-80},{82,80}}, fileName="modelica://HeatPumpModel/../Incons/Logo Ideal Cooler.png")}));
end HeatPump_vs_2_cooling;
