...
model Ref_cycle_vs_4
  "Refrigerant cycle with Daibiri correlation. Smooth function for transition is introduced when a transition Event in off status appears."
...
equation
  ...
  Wel_ref =  smooth( 1, noEvent(if OnOff ==1 then Wel_map * m_ref * (h2_iso - h1)/(m_ref_map * (h2_iso_map - h1)) else 0))                                                                                                                      "eletric power calculated at real operative conditions";
  h2 = smooth(1, noEvent(if OnOff ==1 then Wel_ref/m_ref + h1 else h1)) "enthalpy at compressor outlet";
  ...
  CC_ref = smooth( 1, noEvent(if OnOff ==1 then m_ref*(h1-h4) else 0))
                                                                      "calculation of cooling capacity using the refrigerant cycle";
