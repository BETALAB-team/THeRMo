...
model Ref_cycle_vs_2
  "Ref cycle like TRNSYS type plus evaluation of Dew Point temperatures"
...
//Define Parameters
  ...
  Modelica.Blocks.Interfaces.RealOutput HC_ref( unit = "W")
    "heating capacity calculated with refrigerant cycle" annotation (Placement(
        transformation(extent={{102,36},{144,78}}),  iconTransformation(extent={{102,36},
            {144,78}})));


 parameter Real SH(start = 10, fixed = true, unit ="K") "superheat";
 parameter Real SBC(start = 5, fixed = true, unit = "K") "subcooling";
 ...
 //Real m_ref(  unit ="kg/s") "refigerant mass flow rate";
  ...
  Modelica.Blocks.Interfaces.RealInput m_ref(unit="kg/s")
    "refrigerant mass flow rate" annotation (Placement(transformation(extent={{-124,
            -32},{-100,-8}}),  iconTransformation(extent={{-124,-32},{-100,-8}})));
  Modelica.Blocks.Interfaces.RealOutput CC_ref(unit="W")
    "heating capacity calculated with refrigerant cycle" annotation (Placement(
        transformation(extent={{102,-76},{144,-34}}), iconTransformation(extent={{102,-62},
            {144,-20}})));
