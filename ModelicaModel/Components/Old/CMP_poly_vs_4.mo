...
model CMP_poly_vs_4
  "Introduction of the variable speed polynomials and smooth function"
  ...
  Modelica.Blocks.Interfaces.RealOutput Wel(unit="W") "electrical power"  annotation (Placement(transformation(extent={{70,62},
            {100,92}}),iconTransformation(extent={{70,62},{100,92}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput m_ref(unit="kg/s") "refrigerant mass flow rate"
                                                                                       annotation (Placement(transformation(extent={{68,-78},
            {100,-46}}),iconTransformation(extent={{68,-78},{100,-46}})));
  Modelica.Blocks.Interfaces.IntegerOutput Mode_Output = Mode annotation (Placement(
        transformation(extent={{68,-16},{104,20}}),
                                                  iconTransformation(extent={{68,-16},
            {104,20}})));
  //String parameter
...
equation
  if CMP_type == "Fixed speed" then
    ...
    CC = smooth(1,noEvent(if Mode==1 then -1*cc_coef*t_vector_fix else 0));
    HC = -1*CC + Wel;
  ...
  elseif CMP_type == "Variable speed 30 coeff" then
    ...
    CC = smooth(1,noEvent(if Mode==1 then -1*cc_coef_30*t_vector_30 else 0));
    HC = -1*CC + Wel;
