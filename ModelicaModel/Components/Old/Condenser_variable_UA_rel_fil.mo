...
model Condenser_variable_UA_rel_fil
  "Used relative mass flow rates and relative KA + fil on output temperature"
  ...
  parameter String fileName= fileName_cond;
   ...
   Modelica.Blocks.Continuous.FirstOrder firstOrder(
    T(displayUnit="min") = T_cost_cond,
    initType=Modelica.Blocks.Types.Init.SteadyState,
    y_start=Medium.T_default)
    annotation (Placement(transformation(extent={{70,-40},{90,-20}})));
  parameter Modelica.Units.SI.Time T_cost_cond(displayUnit="min") = 900
...
equation
  ...
  connect(realExpression.y, firstOrder.u)
    annotation (Line(points={{57,-30},{68,-30}}, color={0,0,127}));
...
end Condenser_variable_UA_rel_fil;
