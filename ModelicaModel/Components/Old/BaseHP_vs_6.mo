...
model BaseHP_vs_6 "Variable compressor speed and smooth function"
  ...
  CMP_poly_vs_4 CMP_4(fileName=fileName, CMP_type=CMP_type)
  ...
  OnOff_vs_2 onOff_vs_2
    annotation (Placement(transformation(extent={{-90,-6},{-70,14}})));
  Modelica.Blocks.Interfaces.RealInput CMP_frequency1
    annotation (Placement(transformation(extent={{-148,-12},{-108,28}})));
equation
  ...
  connect(CMP_4.HC, Condenser.HeatFlow) annotation (Line(points={{12,22.88},{12,
          28},{-0.25,28},{-0.25,41.25}}, color={0,0,127}));
  connect(HC,CMP_4. HC) annotation (Line(points={{120,50},{92,50},{92,28},{12,28},
          {12,22.88}},       color={0,0,127}));
  connect(CMP_4.Wel, Wel) annotation (Line(points={{24.25,15.86},{92,15.86},{92,
          24},{118,24}},
                color={0,0,127}));
  connect(CMP_4.m_ref, m_ref) annotation (Line(points={{24,-9.16},{92,-9.16},{92,
          -28},{120,-28}},
                      color={0,0,127}));
  connect(CMP_4.CC, Evaporator.HeatFlow) annotation (Line(points={{14.5,-16.72},
          {14.5,-28},{1.75,-28},{1.75,-45.25}},
                                          color={0,0,127}));
  connect(CMP_4.Tcond, Condenser.RefT) annotation (Line(points={{-27,17.12},{
  ...
  connect(gain.u, Evaporator.HeatFlow) annotation (Line(points={{78.6,-59},{
          78.6,-48},{80,-48},{80,-28},{1.75,-28},{1.75,-45.25}},
                                                            color={0,0,127}));
  connect(DewT.T_dew,CMP_4. Teva) annotation (Line(points={{-41,-16.2},{-42,
  ...
  connect(CMP_4.Mode_Output, Mode_Output) annotation (Line(points={{24.5,2.36},
          {104,2.36},{104,0},{112,0}},     color={255,127,0}));
  connect(onOff_vs_2.OnOff, CMP_4.Mode) annotation (Line(points={{-68,4},{-38,4},
          {-38,0.2},{-27.5,0.2}}, color={255,127,0}));
  connect(onOff_vs_2.Output, CMP_4.CMP_f) annotation (Line(points={{-68.2,-1.2},
          {-58,-1.2},{-58,10.28},{-27,10.28}}, color={0,0,127}));
  connect(onOff_vs_2.CMP_frequency, CMP_frequency1) annotation (Line(points={{
          -93.2,6.8},{-104,6.8},{-104,8},{-128,8}}, color={0,0,127}));
