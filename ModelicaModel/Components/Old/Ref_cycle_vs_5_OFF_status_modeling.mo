...
model Ref_cycle_vs_5_OFF_status_modeling "Introduction of T2 max value"
...
//Define Parameters
  Modelica.Blocks.Interfaces.RealInput Tcond(unit="K", displayUnit ="degC")
    ...
    annotation (Placement(transformation(extent={{-124,78},{-100,102}}),
        iconTransformation(extent={{-124,78},{-100,102}})));
  Modelica.Blocks.Interfaces.RealInput Teva(unit="K", displayUnit ="degC")
    ...
    annotation (Placement(transformation(extent={{-124,-100},{-100,-76}}),
        iconTransformation(extent={{-124,-100},{-100,-76}})));
  Modelica.Blocks.Interfaces.RealInput Wel_map(unit = "W") "compressor power"
    annotation (Placement(transformation(extent={{-124,16},{-100,40}}),
        iconTransformation(extent={{-124,16},{-100,40}})));
  ...
  Modelica.Blocks.Interfaces.RealInput CC_map(unit = "W") "cooling capacity"
    annotation (Placement(transformation(extent={{-124,-68},{-100,-44}}),
        iconTransformation(extent={{-124,-68},{-100,-44}})));
  Modelica.Blocks.Interfaces.RealOutput HC_ref( unit = "W")
    ...
    annotation (Placement(
        ...
        rotation=-90,
        origin={1,123}),                             iconTransformation(extent={{21,-21},
            {-21,21}},
        rotation=-90,
        origin={1,123})));
  Modelica.Blocks.Interfaces.RealInput m_ref_map(unit="kg/s")
     ...
     annotation (Placement(transformation(extent={{-124,-42},{-100,-18}}),
                               iconTransformation(extent={{-124,-42},{-100,-18}})));
  Modelica.Blocks.Interfaces.RealOutput CC_ref(unit="W")
    "heating capacity calculated with refrigerant cycle" annotation (Placement(
        transformation(extent={{102,-76},{144,-34}}), iconTransformation(extent={{-21,-21},
            {21,21}},
        rotation=270,
        origin={1,-123})));
  ...
  Modelica.Blocks.Interfaces.RealOutput COP
    "Coeffiicent of performance of the Heat Pump" annotation (Placement(
        transformation(extent={{102,-110},{144,-68}}),iconTransformation(extent={{102,-50},
            {144,-8}})));
 ...
 Real Poff(unit = "Pa") "Off saturation pressure";
 //Entropies
 ...
 Real T2_unb(  unit ="K") "unbonded temperature at compressor outlet";
 ...
 Medium.ThermodynamicState state2_unb "thermodynamic status at compressor outlet";
 ...
 Real Wel_ref_unb(  unit = "W") "unbonded refrigerant power";
  ...
  Modelica.Blocks.Interfaces.RealInput T_ext "External temperature" annotation
    (Placement(transformation(
        extent={{-15,-15},{15,15}},
        rotation=90,
        origin={79,-115}), iconTransformation(
        extent={{-15,-15},{15,15}},
        rotation=90,
        origin={79,-115})));
equation

  // Calculation of saturation pressure at external conditions
  Poff = Medium.saturationPressure(T_ext) "Liquid Pressure assumed by the refrigerant when the circuit shus down";

  //Calculation of saturation pressures and cmp inlet conditions"
//   T4 = Teva "temperature at the evaporator inlet";
//   Peva =smooth(1,noEvent(if OnOff ==1 then Medium.saturationPressure(T4) else Poff)) "assumed Peva = P4_ls, deltaP neglected";
...
//   T1 = smooth(1,noEvent(if OnOff ==1 then T1_vs + SH else T1_vs + 1e-3))  "suction temperature";
//   T1_map = smooth(1,noEvent(if OnOff ==1 then T1_vs + SH_map else T1_vs + 1e-3)) "suction temperature in standardised conditions";

  T1_vs = Teva "temperature at the evaporator inlet";
  T1 = smooth(1,noEvent(if OnOff ==1 then T1_vs + SH else T1_vs + 1e-2))  "suction temperature";
  T1_map = smooth(1,noEvent(if OnOff ==1 then T1_vs + SH_map else T1_vs + 1e-2)) "suction temperature in standardised conditions";
  ...
  Peva= smooth(1,noEvent(if OnOff ==1 then dewState1.p else Poff))  "assumed Pcond = P2_vs, deltaP neglected";

  //Enthalpy point 1
  h1 = smooth(1,noEvent(if OnOff ==1 then Medium.specificEnthalpy_pT(Peva,T1) else h1_vs)) "suction enthaply";
  ...
  Pcond = smooth(1,noEvent(if OnOff ==1 then dewState2.p else Peva))
                                                                    "assumed Pcond = P2_vs, deltaP neglected";
  ...
  T3 = smooth(1,noEvent(if OnOff ==1 then sat3.Tsat - SBC else sat3.Tsat + 1e-2)) "temperature at the condenser outlet";
  //T3 = sat3.Tsat - SBC "temperature at the condenser outlet";
  h3 = smooth(1,noEvent(if OnOff ==1 then Medium.specificEnthalpy_pT(Pcond,T3) else sat3.hl)) "enthalpy at the condenser outlet";
  h4 = h3 "isoenthalpic lamination";
  T4 = smooth(1,noEvent(if OnOff ==1 then Medium.temperature_ph(Peva,h4) else sat1.Tsat)) "enthalpy at the condenser outlet";

  //Energy balance
  m_ref = smooth(1,noEvent(if OnOff ==1 then m_ref_map *(1+ F*(d1/d1_map - 1)) else 0)) "refrigerant mass flow rate calculated at real operative conditions";
  Wel_ref_unb =  smooth( 1, noEvent(if OnOff ==1 then Wel_map * m_ref * (h2_iso - h1)/(m_ref_map * (h2_iso_map - h1)) else 0)) "unbonded eletric power calculated at real operative conditions";
  h2_unb = smooth(1,noEvent(if OnOff ==1 then Wel_ref_unb/m_ref + h1 else h1)) "enthalpy at compressor outlet";
  state2_unb = Medium.setState_ph(Pcond,h2_unb,1) "thermodynamic status at compressor outlet";
  T2_unb =  state2_unb.T "unbonded temperature at the compressor outlet";
  T2 = Buildings.Utilities.Math.Functions.smoothMin(T2_unb, 105 + 273.15,1e-4)  "impose limitation for the discharge temperature, if off the temperature collapses on saturated vapor";
  h2 = smooth( 1,noEvent(if OnOff ==1 then Medium.specificEnthalpy_pT(Pcond,T2,1) else h1)) "real entalpy at the condenser outlet, if off it collapses on h1";
  Wel_ref = smooth( 1, noEvent(if OnOff ==1 then m_ref*(h2-h1) else 0))
                                                                       "calculation of cmp power using the refrigerant cycle";
  HC_ref = smooth( 1, noEvent(if OnOff ==1 then m_ref*(h2-h3) else 0)) "calculation of heating capacity using the refrigerant cycle";
  CC_ref = smooth( 1, noEvent(if OnOff ==1 then m_ref*(h1-h4) else 0)) "calculation of cooling capacity using the refrigerant cycle";
  COP = smooth( 1, noEvent(if OnOff ==1 then HC_ref / Wel_ref else 0)) "Coefficient of performance";

  //Calculation of percentage of heat exchanged in condensation
  hcond_vs = sat3.hv;
  hcond_ls = sat3.hl;
  Deltah_cond = (hcond_vs - hcond_ls)/(h2 - h3)*100;
  annotation (Icon(graphics={
        ...
        Text(
          ...
          textString="%name
")}));
end Ref_cycle_vs_5_OFF_status_modeling;
