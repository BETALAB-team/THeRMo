...
model Evaporator_latent_fraction
  ...
  Modelica.Blocks.Interfaces.RealInput HeatFlow(unit="W") annotation (Placement(
        ...
        rotation=90,
        origin={-37,-107})));
  Modelica.Blocks.Sources.RealExpression realExpression(y=SensEF.T + HeatFlow *X_latent
  ...
  Modelica.Blocks.Interfaces.RealInput X_latent
    "Percentage of heat exchanged with latent" annotation (Placement(
        transformation(
        extent={{-11,-11},{11,11}},
        rotation=90,
        origin={-9,-107}), iconTransformation(
        extent={{-11,-11},{11,11}},
        rotation=90,
        origin={15,-107})));
equation
  NTU = UA_nom/(port_a.m_flow*cp);
  ...
  connect(realExpression.y, RefT)
    annotation (Line(points={{63,-52},{110,-52}}, color={0,0,127}));
...
end Evaporator_latent_fraction;
