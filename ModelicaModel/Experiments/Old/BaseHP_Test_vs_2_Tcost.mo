...
model BaseHP_Test_vs_2_Tcost "Test BaseHP with constant input"
  import Modelica;
  ...
  Modelica.Blocks.Sources.RealExpression Wel(y=1)
    annotation (Placement(transformation(extent={{-72,-12},{-48,10}})));
equation
  connect(realToInteger1.y, baseHP_vs_3.OnOff) annotation (Line(points={{-27.5,
          -1},{-27.5,-2},{-6,-2},{-6,-2},{-3.12,-2}},       color={255,127,0}));
  connect(Load.ports[1], baseHP_vs_3.Load_in) annotation (Line(points={{-22,28},
          {9.62,28},{9.62,11.2}},  color={0,127,255}));
  connect(Source.ports[1], baseHP_vs_3.Source_in) annotation (Line(points={{-24,-32},
          {10.4,-32},{10.4,-15.44}},     color={0,127,255}));
  connect(Load_Volume.ports[1], baseHP_vs_3.Load_out) annotation (Line(points={{20,58},
          {28,58},{28,11.2},{16.9,11.2}},            color={0,127,255}));
  connect(Source_Volume.ports[1], baseHP_vs_3.Source_out) annotation (Line(
        points={{44,-48},{48,-48},{48,-20},{18.2,-20},{18.2,-15.44}}, color={0,
          127,255}));
  connect(baseHP_vs_3.Source_out, T_source_out.port) annotation (Line(points={{18.2,
          -15.44},{18.2,-20},{48,-20},{48,-30},{80,-30},{80,-24}},      color={
          0,127,255}));
  connect(T_load_out.port, baseHP_vs_3.Load_out) annotation (Line(points={{78,18},
          {78,11.2},{16.9,11.2}},        color={0,127,255}));
  connect(Wel.y, realToInteger1.u)
    annotation (Line(points={{-46.8,-1},{-39,-1}}, color={0,0,127}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio=false)),
    experiment(StopTime=86400, __Dymola_Algorithm="Dassl"));
end BaseHP_Test_vs_2_Tcost;
